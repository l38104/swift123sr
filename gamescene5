...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if body1.categoryBitMask == PhysicsCategories.chaseShip && body2.categoryBitMask == PhysicsCategories.playerShip {
        ...
        if body1.categoryBitMask == PhysicsCategories.border && body2.categoryBitMask == PhysicsCategories.playerShip {
            
        }
        
        if body1.categoryBitMask == PhysicsCategories.turretMissiles && body2.categoryBitMask == PhysicsCategories.playerShip {
    ...
    func walls() {
        ...
        var points = [[Float]]()
        var borderPhysicsBodies = [SKPhysicsBody]()
        ...
        for n in stride(from: 0, to: points.count, by: 1) {
            ...
            borderPhysicsBodies.append(box.physicsBody!)
        }
        
        let conglomeratePhysicsBody = SKPhysicsBody(bodies: borderPhysicsBodies)
        
        conglomeratePhysicsBody.collisionBitMask = PhysicsCategories.none
        conglomeratePhysicsBody.categoryBitMask = PhysicsCategories.border
        conglomeratePhysicsBody.contactTestBitMask = PhysicsCategories.playerShip
    }
    ...
    func turret(turretPosition: CGPoint) {
        ...
        let rotate = SKAction.run {
            ...
            self.shield.position = self.playerShip.position
    ...
    func meteor() {
        ...
        meteor.physicsBody!.contactTestBitMask = PhysicsCategories.playerShipMissile
    ...
    func callMeteor(duration: Int) {
        ...
        let call = SKAction.run { self.meteor() }
    ...
    func playerShield(activate: Bool) {
    ...
    func addObstacle(interval: Int) {
        ...
        for n in stride(from: 0, to: points.count, by: 1) {
            ...
            box.physicsBody = SKPhysicsBody(rectangleOf: box.size)
            box.physicsBody?.isDynamic = false
            wallPhysicsBodies.append(box.physicsBody!)
        }
        
       // let conglomeratePhysicsBody = SKPhysicsBody(bodies: wallPhysicsBodies)
    }
