...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func callBackground() {
        ...
        let call = SKAction.run { self.background(); print("new") }
    ...
    func chaseShipMovement() {
        
        let move = SKAction.run {
            
            let moveAction = SKAction.moveTo(x: self.playerShip.position.x, duration: 0.5)
            self.chaseShip.run(moveAction)
        }
    ...
    func addScoreFunc() {
        ...
        let obstacle = childNode(withName: "obstacleBlock")
        
        let check = SKAction.run {
            
            if self.playerShip.position.y > (obstacle?.position.y)! {
        ...
        let callBack = SKAction.run { if self.call { self.addScoreFunc() } }
        
        let sequence = SKAction.sequence([check, wait, callBack])
    ...
    func runGameOver() {
        ...
        childNode(withName: "background1")?.removeFromParent()
        childNode(withName: "background2")?.removeFromParent()
        ...
        let delete = SKAction.run {
            ...
            self.enumerateChildNodes(withName: "obstacleBlock") {
                node, stop in
                node.removeAllActions()
                node.removeFromParent()
            }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if nodeTapped.name == "start" || nodeTapped.name == "gameOver" {
            ...
            if childNode(withName: "missile") != nil {
                childNode(withName: "missile")?.removeAllActions()
                childNode(withName: "missile")?.removeFromParent()
            }
            ...
            let call = SKAction.run { self.chaseShipMissile() }
            let wait = SKAction.wait(forDuration: 5)
            let sequence = SKAction.sequence([wait, call])
            self.run(sequence)
